...
script_dir="${0:A:h}"
log="$script_dir/probe-rs-gdb-f103-snapshot.log"
reset_args=()
connect_args=()
...
if ! arm-none-eabi-gdb -q -x "$script_dir/snapshot_f103.gdb"; then
  print -u2 -r -- "Failed to snapshot F103. probe-rs gdb log:"
  sed -n '1,180p' "$log" >&2
  exit 1
fi
