...
(function (RW) {
  ...
  const SENSORS = [
    {
      ...
      label: "Temperature",
      unit: (u) => (u === "f" ? "°F" : "°C"),
      fmt: (v, u) => (u === "f" ? (v * 9) / 5 + 32 : v).toFixed(1),
      bar: "°",
    },
    {
      key: "humid",
  ...
  RW.ScoreHero = function ({ status }) {
    ...
    const word =
      sc.color === "green" ? "Good" : sc.color === "amber" ? "Fair" : "Poor";
    return html` <section data-screen-label="Score">
      <div class="card score-hero">
        <div class="score-left">
          <span class="score-cap">
            <span
              class="sdot"
              style=${"background:" + RW.accent(sc.color)}
            ></span
            >Air score</span
          >
        ...
        <div class="bars" aria-label="Per-sensor index bars">
          ${SENSORS.filter((s) => !s.aux).map((s) => {
            ...
            return html` <div class="bar-col" key=${s.key}>
              <div class="bar-track">
                <div
                  class="bar-fill"
                  style=${"height:" +
            ...
            <div class="w-top">
              ...
              ${s.aux
                ...
                : idx > 0
                  ? html`<span
                      class="w-dot"
                      style=${"background:" + RW.accent(RW.indexColor(idx))}
                    ></span>`
                  : null}
            </div>
            <div class="w-val-row">
  ...
  RW.DisplaySelector = function ({ status, cycle, onPick }) {
    ...
    const temp =
      status.settings.units === "f"
        ? (status.sens.temp * 9) / 5 + 32
        : status.sens.temp;
    const cyc = [
      temp.toFixed(0) + "°" + status.settings.units.toUpperCase(),
      String(Math.round(status.sens.humid)) + "%",
    ];
    const clock = now
      ? (now.getHours() % 12 || 12) + ":" + RW.p2(now.getMinutes())
      : "--:--";
    const previews = {
      score: String(status.score.value),
      clock: status.time.valid ? clock : "--:--",
      sensors: cyc[cycle % cyc.length],
    };
    return html` <h2 class="sec-label">Device display</h2>
      <section data-screen-label="Display mode">
        <div class="grid-modes">
          ${["score", "clock", "sensors"].map(
            (m) =>
              html` <button
                class="mode-card"
                key=${m}
              ...
              >
                <span class="matrix"
                  ><${RW.Matrix}
                    text=${previews[m]}
                    bars=${m === "score"
                      ? SENSORS.filter((s) => !s.aux).map(
                          (s) => status.score.indices[s.key],
                        )
                      : null}
                /></span>
                <span class="mode-name"
                  >${m[0].toUpperCase() + m.slice(1)}</span
                >
              </button>`,
          )}
        </div>
