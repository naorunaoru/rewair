...
(function (RW) {
  ...
  RW.fmtOffset = (min) => {
    ...
    return (
      "UTC" + (min < 0 ? "−" : "+") + p2(Math.floor(a / 60)) + ":" + p2(a % 60)
    );
  };
  RW.fmtClock = (d, secs) =>
    p2(d.getHours()) +
  ...
  RW.setEpoch = (epoch) => {
    if (epoch == null) {
      epochBase = null;
      epochSource = null;
    } else if (epochBase === null || epoch !== epochSource) {
      /* Status events can repeat the epoch captured at the last NTP/manual
       * update. Keep the existing local anchor for identical samples so the
       * visible clock advances instead of snapping back on every event. */
      epochBase = epoch;
      epochSource = epoch;
      epochAt = Date.now();
    }
  ...
  RW.deviceNow = (status) => {
    ...
    if (status && status.settings.time_mode === "manual")
    ...
    return new Date(
      (epoch + offs * 60) * 1000 + new Date().getTimezoneOffset() * 60000,
    );
  };
  ...
  RW.netStatus = (net, opts) => {
    ...
    if (net.connected) {
      let s =
        "Connected" +
        (net.connected_s != null ? " " + RW.fmtDuration(net.connected_s) : "");
  ...
  RW.Signal = function ({ net, showDbm }) {
    ...
    const barsEl = html` <span class="rbars sig" style="height:18px">
      ${[1, 2, 3, 4].map(
        (i) =>
          html`<span style=${i <= bars ? `background:${col}` : ""}></span>`,
      )}
    </span>`;
    if (showDbm && net.rssi != null) {
      return html`<div class="wifi-sig">
        ${barsEl}<span class="rssi-small">${net.rssi} dBm</span>
      </div>`;
    }
    return barsEl;
  };

  RW.NetworkRow = function ({
    net,
    size = "md",
    lead,
    trailing,
    ...
    return html` <div
      class=${"nrow" +
      (net.in_range || net.connected ? "" : " dim") +
      (cls ? " " + cls : "")}
      style=${big ? "border:0;padding:6px 2px" : ""}
    >
      ${lead}
  ...
  const GLYPHS = {
    ...
    "-": ["000", "000", "111", "000", "000"],
    "%": ["011001", "011010", "000100", "001011", "010011"],
    " ": ["0", "0", "0", "0", "0"],
  };
  const SCORE_DIGITS = {
  ...
  function drawMatrix(canvas, text) {
    ...
    for (let r = -pad; r < rows + pad; r++)
      for (let c = -pad; c < cols + pad; c++)
        dot(ctx, c + pad, r + pad, off, cell, gap);
    let x0 = pad;
    glyphs.forEach((g) => {
  ...
  function drawScoreBars(canvas, text, bars) {
    const rows = 5,
      cell = 4,
      gap = 2,
      pad = 1;
    const glyphs = [...text].map(
      (ch) => SCORE_DIGITS[ch] || GLYPHS[ch] || GLYPHS[" "],
    );
    const textCols = glyphs.reduce(
      (sum, glyph, i) => sum + glyph[0].length + (i ? 1 : 0),
      0,
    );
    const barsCols = bars.length * 2 - 1;
    const textStart = barsCols + 3;
    const cols = textStart + textCols;
    const w = (cols + pad * 2) * (cell + gap) - gap;
    const hgt = (rows + pad * 2) * (cell + gap) - gap;
    const dpr = window.devicePixelRatio || 1;
    canvas.width = w * dpr;
    ...
    for (let r = -pad; r < rows + pad; r++)
      for (let c = -pad; c < cols + pad; c++)
        dot(ctx, c + pad, r + pad, off, cell, gap);
    let x0 = textStart + pad;
    glyphs.forEach((glyph) => {
      for (let r = 0; r < rows; r++)
        for (let c = 0; c < glyph[r].length; c++)
          if (glyph[r][c] === "1") dot(ctx, x0 + c, r + pad, led, cell, gap);
      x0 += glyph[0].length + 1;
    });
    bars.forEach((index, c) => {
      const height = Math.max(1, Math.min(rows, Number(index) + 1));
      for (let r = rows - height; r < rows; r++)
        dot(ctx, c * 2 + pad, r + pad, led, cell, gap);
    });
  }
  RW.Matrix = function ({ text, bars }) {
    const ref = useRef();
    useEffect(() => {
      bars
        ? drawScoreBars(ref.current, text, bars)
        : drawMatrix(ref.current, text);
    }, [text, bars]);
